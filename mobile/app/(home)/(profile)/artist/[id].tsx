...
import BioModal from '@/components/artist/BioModal'
...
export default function ArtistScreen() {
  ...
  const openCreateCommunitySheet = useCallback(() => {
    if (!artist || !id) return
    router.push({
      pathname: '/(home)/(profile)/artist/create-community',
      params: {
        artistId: id,
  ...
  const openAllCommunitiesScreen = useCallback(() => {
    if (!id) return
    router.push({
      pathname: '/(home)/(profile)/artist-communities',
      params: {
        id,
        artistName: artist?.name ?? '',
      },
  ...
  const visibleCommunities = communityCards.slice(0, 2)
    ...
    <View className="flex-1 bg-surface">
      ...
      {loading ? (
        ...
        >
          ...
          >
            {/* Hero */}
            <View className="relative overflow-hidden" style={{ height: screenHeight * 0.55, backgroundColor: '#000' }}>
                  ...
                  {isLiquidGlassAvailable() ? (
                    <GlassView isInteractive tintColor="rgba(186, 0, 43, 0.6)" style={{ borderRadius: 50 }}>
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                      >
                        <SymbolView name="plus" size={22} tintColor={Colors.primary} />
                      </TouchableOpacity>
                    </GlassView>
                  ) : (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                    >
                      <SymbolView name="plus" size={24} tintColor={Colors.primary} />
                    </TouchableOpacity>
                  )}
                  {communityCards.length > 2 ? (
                    <TouchableOpacity
                      onPress={openAllCommunitiesScreen}
                      activeOpacity={0.7}
                    >
                    ...
                    {visibleCommunities.slice(0, 2).map((c) => {
                      ...
                      return (
                        <CommunityCard
                          key={c.id}
                          community={c}
                          href={{ pathname: '/(home)/(profile)/community/[id]', params: { id: targetId } }}
                          replace
                          withAppleZoom
                        />
                      )
      ...
      {/* Info button */}
      {artist && isLiquidGlassAvailable() ? (
        <View
          className="absolute"
          style={{ right: 16, bottom: 30, zIndex: 10 }}
        >
          <GlassView isInteractive style={{ borderRadius: 50 }}>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <SymbolView name="info" size={24} tintColor="black" />
            </TouchableOpacity>
          </GlassView>
        </View>
      ) : null}
