...
function musicBadgeNeedsBg(containerColor?: string): boolean {
  ...
  // Need backdrop when the container luminance is close to Apple Music red's (~0.402)
...
function MusicSyncBadge({ containerColor }: { containerColor?: string }) {
  ...
  return (
    <div
      ...
      title="Synced from Apple Music"
    >
      <span className="material-symbols-outlined" style={{ fontSize: 11, color: "#FA243C" }}>music_note</span>
      <span className="text-[10px] font-semibold" style={{ color: "#FA243C" }}>Apple Music</span>
    </div>
  );
}
